...
class Jove(BeetsPlugin):
    """
    Main plugin class
    """

    def __init__(self):
        super(Jove, self).__init__()
    ...
    def fill(self, ids, lb, ub):
        ...
        conn = sqlite3.connect(config["jove"]["db"].get("unicode"))
        cur = conn.cursor()
    ...
    def commands(self):
        ...
        features_cmd = Subcommand(
            "features",
            help="Generate sequential acoustic features [jove]")
        ...
        def features_func(lib, opts, args):
            ...
            seq_features_file = config["jove"]["features"].get("unicode")
        ...
        coords_cmd = Subcommand(
            "coords",
            help=
            "Generate coordinates based on calculated features [jove]")
        ...
        def coords_func(lib, opts, args):
            if opts.type:
                seq_features_file = config["jove"]["features"].get(
                    "unicode")
                seq_features = cPickle.load(open(seq_features_file, "rb"))
                ...
                if opts.type == "mean":
                    ...
                    model = LSTMSeq2Seq(
                        config["jove"]["model"]["arch"].get("unicode"),
                        config["jove"]["model"]["weights"].get("unicode"),
                        config["jove"]["model"]["output"].get())
                    # Pad sequences
                    maxlen = 150
                ...
                print("Writing to db...")
                conn = sqlite3.connect(config["jove"]["db"].get(
                    "unicode"))
                cur = conn.cursor()
                cur.execute("DELETE FROM coords")

                to_insert = []
                for idx in range(features_2d.shape[0]):
    ...
    def clean(self, lib):
        ...
        # Remove items not in beets db from jove db
        ids_to_remove = []
        for idx in ids:
    ...
    def random_coords(self, lib):
        """
        Update coordinates in jove database after beets db change
        """
